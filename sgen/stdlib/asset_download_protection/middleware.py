...
SCRIPT_STR = """async function l(){try{const h=[],m={};for(var f of document.querySelectorAll("*[data-prot-src-id]:not(img)")){const a=h[f.dataset.protSrcId],b=await (await fetch(a)).blob();let c=URL.createObjectURL(b.slice(1,b.size,b.type));c=a.includes("#")?c+"#"+a.split("#").slice(-1)[0]:c;f.addEventListener("load",()=>URL.revokeObjectURL(c));void 0!==f.src?f.src=c:void 0!==f.getAttribute("xlink:href")&&f.setAttribute("xlink:href",c)}for(const a of document.querySelectorAll("img[data-prot-src-id]")){a.addEventListener("contextmenu",
d=>d.preventDefault());a.addEventListener("mousedown",d=>d.preventDefault());a.style["pointer-events"]="none";const b=document.createElement("img");b.src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAAAXNSR0IArs4c6QAAAAtJREFUGFdjYAACAAAFAAGq1chRAAAAAElFTkSuQmCC";const c=document.createElement("div");for(const d of a.style)c.style[d]=a.style[d];c.style.position="relative";b.style.position="absolute";b.style.width="100%";b.style.height="100%";b.style.top="0";b.style.bottom="0";
b.style.left="0";b.style.right="0";b.alt="Dummy image";b.addEventListener("contextmenu",d=>d.preventDefault());b.addEventListener("mousedown",d=>d.preventDefault());a.parentNode.insertBefore(c,a);for(const d of m[h[a.dataset.protSrcId]]){const e=document.createElement("img");f=d;const k=await (await fetch(f)).blob();let g=URL.createObjectURL(k.slice(1,k.size,k.type));g=f.includes("#")?g+"#"+f.split("#").slice(-1)[0]:g;e.addEventListener("load",()=>URL.revokeObjectURL(g));void 0!==e.src?e.src=g:void 0!==
a.getAttribute("xlink:href")&&a.setAttribute("xlink:href",g);e.style.position="absolute";e.style.width="100%";e.style.height="100%";e.style.top="0";e.style.bottom="0";e.style.left="0";e.style.right="0";e.alt=a.alt;c.append(e)}a.remove();c.append(b)}}catch(h){console.error(h)}}"loading"===document.readyState?document.addEventListener("DOMContentLoaded",()=>l()):l();"""  # noqa:E501
